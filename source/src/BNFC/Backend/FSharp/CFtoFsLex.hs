...
cMacros =
  ...
  , "let digit  = ['0'-'9']                                                         (*  _digit *)"
  , "let idchar = _letter | _digit | ['_' '\\'']                                    (*  identifier character *)"
  , "let universal = ['\\000'-'\\255']                                              (* universal: any character *)"
  , ""
  ]
...
rules cf = mkRule "token" $
    ...
    ++
    -- integers
    [ ( "_digit+", "TOK_Integer (int (lexeme lexbuf))" )
    -- doubles
    , ( "_digit+ '.' _digit+ ('e' ('-')? _digit+)?"
      , "TOK_Double (float (lexeme lexbuf))" )
    -- strings
    , ( "'\\\"' (([^ '\\\"' '\\\\' '\\n']) | ('\\\\' ('\\\"' | '\\\\' | '\\\'' | 'n' | 't' | 'r')))* '\\\"'"
